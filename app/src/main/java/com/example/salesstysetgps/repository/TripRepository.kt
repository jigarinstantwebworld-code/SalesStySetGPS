...
import com.example.salesstysetgps.util.TripData
...
//            emit(Resource.Error(errorMessage))
    ...
    fun endTrip(
        ...
        try {
            ...
            val endTime = dateFormat.format(java.util.Date())
...
//
    ...
    suspend fun fetchTodaysLeads(page:String,limit: String,currentDate: String): Resource<LeadsResponse> {
        return try {
            ...
            val request = PaginationRequest(page, limit , salesExecutiveId.toString(), startDate = currentDate, endDate = currentDate)
            val response = withContext(Dispatchers.IO) {
